...
wallCollision gs = (xCollision, yCollision)
  where
    ...
    xCollision = null $ filter (\(wx, wy, ww, wh) -> ((x + ballradius > wx - ww) || (x - ballradius < wx + ww)) && y <= wy + wh/2 && y >= wy - wh/2) ws
    yCollision = null $ filter (\(wx, wy, ww, wh) -> ((y + ballradius > wy - wh) || (y - ballradius < wy + wh)) && x <= wx + ww/2 && x >= wx - ww/2) ws
