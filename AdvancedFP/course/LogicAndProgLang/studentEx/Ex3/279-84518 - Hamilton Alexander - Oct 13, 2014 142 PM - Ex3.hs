...
--Tableau proof (pick alphas first): False , 5909
--(...The 3rd line took about 10 minutes to produce after the 2nd was output;
-- and the program  was not run long enough to produce the next output).
test = do
    xs <- sample' arbitrary
    mapM checkPrint xs
