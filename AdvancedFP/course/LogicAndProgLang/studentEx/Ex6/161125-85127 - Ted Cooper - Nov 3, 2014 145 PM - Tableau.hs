...
deriving instance (Ord f, Ord v) => Ord (Term f v)
...
tabTree :: [FormulaS] -> Tree -> State (TableauState String String) Tree
tabTree [] tree = return tree
tabTree (x:xs) tree =
  case discrim x of
    Lit p -> tabTree xs tree
    Alpha a b -> tabTree (a:b:xs) (extendTree (double a b) tree)
    Beta a b -> do pushPath
                   x <- tabTree (a:xs) (single a)
                   popPath
                   pushPath
...
xformTree :: Tree -> State (TableauState String String) Tree
xformTree (Direct form tree) = do ts    <- gets path
                                  let fts = nub $ freshvars form
                                  modify $ \s -> s { path = nub $ freshvars form ++ ts }
                                  s     <- getPathSubst
                                  tree' <- xformTree tree
                                  return $ Direct (subst s form) tree'
xformTree (Branch t1 t2)     = do pushPath
                                  t1' <- xformTree t1
                                  popPath
                                  advancePathNames
                                  pushPath
                                  ...
                                  advancePathNames
                                  return $ Branch t1' t2'
xformTree tree               = return tree

-- only the generated ones
freshvars :: FormulaS -> [Term String String]
freshvars = filter (\(Var v) -> last v >= '0' && last v <= '9') . vars

startTermMap = do tm <- gets t2w
                  tms' <- forM (Map.toList tm) $ \(t, i) -> do
                    ts <- forM [1..i] (const freshTerm)
                    return (t, ([], ts))
                  modify $ \s -> s { t2r = Map.fromList tms' } 

advancePathNames = do ts <- gets path
                      forM ts $ \t -> do
                        tm <- gets t2r
                        case Map.lookup t tm of 
                             Just (ts1, [])       -> error $ "trying to advance exhausted name " 
                                                          ++ show t ++ " past " ++ show ts1
                             Nothing              -> error $ "trying to advance untracked name " 
                                                         ++ show t
                             Just (ts1, (t':ts2)) -> do modify $ \s -> s { t2r = Map.insert t (ts1 ++ [t'], ts2) tm }

getPathSubst :: State (TableauState String String) (Subst String (Term String))
getPathSubst = do ts <- gets path
                  ss <- forM (ts :: [Term String String]) (\t@(Var v) -> getPathSynonym t >>= return . (v |->))
                  return $ foldr (|=>) emptySubst ss


getPathSynonym t = do tm <- gets t2r
                      case Map.lookup t tm of
                           Just (_, [])   -> error $ "no fresh names left for " ++ show t
                           Just (_, t':_) -> return t'
                           Nothing        -> error $ "trying to look up untracked name " ++ show t


data TableauState f v = TableauState { i      :: Int
                                     , path   :: [Term f v]
                                     , t2w    :: Map.Map (Term f v) Int
                                     , t2r    :: Map.Map (Term f v) ([Term f v], [Term f v])
                                     , pStack :: [Int]
                                     } deriving (Show, Eq)

emptyTableauState = TableauState { i = 1, path = [], t2w = Map.empty, t2r = Map.empty, pStack = [] }

prependToPath :: (Eq f, Eq v) => Term f v -> State (TableauState f v) (Term f v)
prependToPath t = do ts    <- gets path
                     modify $ \s -> s { path = t : ts }
                     return t

pushPath :: (Eq f, Eq v) => State (TableauState f v) Int
pushPath = do ts    <- gets path
              ls    <- gets pStack
              let l  = length ts
              modify $ \s -> s { pStack = l : ls }
              return l

popPath :: (Eq f, Eq v) => State (TableauState f v) [(Term f v)]
popPath = do ts <- gets path
             ls <- gets pStack
             let l = length ts
             case ls of []       -> error "tried to pop empty stack"
                        (ol:ls') -> let (ds, ts') = splitAt (l - ol) ts
                                    in do modify $ \s -> s { path   = ts'
                                                           , pStack = ls' }
                                          return ds

widenPath :: (Eq f, Eq v, Ord f, Ord v) 
          => State (TableauState f v) ()
widenPath = do ts     <- gets path
               tm     <- gets t2w
               let adj = foldl (.) id $ map (Map.adjust (+1)) ts
               modify  $ \s -> s { t2w = adj tm }

freshInt :: (Eq f, Eq v) => State (TableauState f v) Int
freshInt = do { n <- gets i; modify $ \s -> s { i = n+1 }; return n}

freshTerm = do n <- freshInt
               return $ Var ("n"++show n)

freshTrackedTerm = do t <- freshTerm    
                      tm    <- gets t2w
                      modify $ \s -> s { t2w = Map.insert t 1 tm }
                      prependToPath t   

freshSkolem :: String -> FormulaS -> State (TableauState String String) FormulaS
freshSkolem v form =
  do n       <- freshInt
...
try :: FormulaS -> Block
try x = above (tblock tree) (oneBlock $ show next)
  where (tree,next) = runState m emptyTableauState
        m           = do t <- tabTree [notP x] (single (notP x))
                         startTermMap
                         xformTree t
...
tab3:: [FormulaS] -> [[FormulaS]] -> State (TableauState String String) [[FormulaS]]    
...
try2 x = runState (tab3 [notP x] [[]]) emptyTableauState
