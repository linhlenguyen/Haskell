...
instance Functor (Store s) where
  fmap f (St g) = St h 
    where h s = case (g s) of { (x,s2) -> (f x,s2)}

instance Applicative (Store s) where
  pure x = St(\ e -> (x,e))
  (<*>) (St f) (St g) = St h
    where h e = case f e of { (f2,s2) -> case g s2 of { (y,s3) -> (f2 y,s3)}} 
...
instance Applicative Mult where
  pure x = (Mult [x])
  (<*>) (Mult fs) (Mult xs) = Mult [ f x | f <- fs, x <- xs]
...
instance Functor Output where
   fmap f (OP(x,str)) = OP(f x,str)

instance Applicative Output where
   pure x = OP(x,"")
   (<*>) (OP(f,s1)) (OP(x,s2)) = OP(f x,s1 ++ s2)
...
-----------------------------------------------------
-- Super Duper all encompassing Monad
...
instance Functor (M v) where
  fmap f (M g) = M h
    where h st env = case g st env of
                       (mx,str,mp) -> (fmap f mx,str,st)
  ...
  (<*>) (M f) (M g) = M h
    where h st env = 
             case f st env of
               (Just f2,str1,st2) ->
                  case g st2 env of
                    (Just y,str2,st3) -> (Just (f2 y),str1++str2,st3)
                    (Nothing,str,st3) -> (Nothing,str,st3)
               (Nothing,str,st2) -> (Nothing,str,st2)
