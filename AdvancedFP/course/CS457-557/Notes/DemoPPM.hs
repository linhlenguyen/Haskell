...
go5 = let f x = x*x in plotFun f "Pics/quadFun.ppm"
go6 = let f x = x*x*x - 2*x*x in plotFun f "Pics/cubicFun.ppm"
...
f10 = draw "Pics/f10.ppm" yellow pict (-4,-4) (4,4) (400,400)
  where pict = layer [ translate (1,1) (circle red 2)
                     , translate (-1,-1) (circle blue 1)
                     , axis
                     , checks white yellow]
