module MonadInterpWorksheetDiscussion where

data Store s x = St(s -> (x,s))

eval:: Store a b -> a -> (b,a)
eval (St f) init = f init

readS v = St f
  where f dict = (find v dict,dict)
writeS v x =  St  g
  where g dict = (lookup v dict,update v x dict)

ret x = St f
  where f dict = (x,dict)

increment = St f
 where f initial = (initial, initial + 1)
...
eval5 (Sub5 x y) = do {x' <- eval5 x; y' <- eval5 y; return (x' - y')}
eval5 (Mult5 x y) = do {x' <- eval5 x; y' <- eval5 y; return (x' * y')}
eval5 (Int5 n) = ret n
eval5 (Var5 s) = readS s
eval5 (Assign5 s x) = do { x' <- eval5 x; writeS s x'; return x'}

p1 = Sub5 (Add5 (Assign5 "x" (Int5 12)) (Int5 3)) (Var5 "x")
p2 = Add5 (Var5 "x") p1 
------------------------------
