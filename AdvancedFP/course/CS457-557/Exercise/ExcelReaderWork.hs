...
import Excell(Table,col,rows,cols,stack,above,beside,blankRow)
import Control.Monad.State(evalState,execState)

tests1 = ["Tom","34","True","3","6","7"]

ex1 = evalState (string `x` int `x` string) tests1
ex1b = execState (string `x` int `x` string) tests1
...
ex2b = execState (string `x` int `x` bool `x` list 3 int `x` int) tests1
...
getfile = 
  do { Right listOflistOfString <- parseCSVFromFile "DemoMajors.csv"
     ; return (evalState undefined listOflistOfString)
     }
