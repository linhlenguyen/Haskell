...
data Map = M (DM.Map Var (Value2 Map))

instance Environment Map where
  look (M e) var = e DM.! var
  extend (M env) var value = M(DM.insert var value env)
  extpat (M env) (Pat x y) (Prod2 u v) = 
     M(DM.insert x u (DM.insert y v env))
  extpat env pat val = error ("Bad pair value in extpat")
  


{-
extpat :: Environment e => e -> Pat -> Value2 e -> e
*Main> :i Pat
data Pat = Pat Var Var
        -- Defined at D:\work\sheard\Courses\AdvancedFP\web\co
*Main> :i Value2
data Value2 e
  = Int2 Int | Fun2 Var Term2 e | Prod2 (Value2 e) (Value2 e)
-}
