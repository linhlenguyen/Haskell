...
myStyle = 
  ...
  , reservedOpNames =
    ["<", "=", "+", "-", "*"]
  , reservedNames  = 
    ["if","then","else", "while","do","call","begin", "end","True","False"]
  }                    
...
simpleP:: MParser Exp
simpleP = (var <|> bool <|> int <|> parenS expP) <?> "<simple exp>"
  where var = fmap Var idenT
        int = do { n <- int32; return(Int n)}
        int2 = int32 >>= (return . Int)
        bool = (keyworD "True" >> return (Bool True)) <|> 
...
expP:: MParser Exp
expP = (chainl1 term compareop) <?> "<exp>"
...
assignP = 
  (do { x <- idenT
     ; symboL ":="
     ; e <- expP
     ; return (Assign x e)}) <?> "<assignment stmt>"
...
stmtP = (whileP <|> ifP <|> callP <|> blockP <|> assignP) <?> "<stmt>"

-------------------------------------------
-- declarations

declP = (valP <|> funP) <?> "<decl>"
