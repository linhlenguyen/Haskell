...
numeric (Exception _ _ _) name fun x y = Bad
numeric st name fun (IntV x) (IntV y) = IntV(fun x y)
numeric st name fun v (IntV _) = error ("First arg of "++name++" is not an Int. "++show v)
numeric st name fun (IntV _) v = error ("Second arg of "++name++" is not an Int. "++ show v)
numeric st name fun u v = error ("neither arg of "++name++" is an int: "++show u++" "++show v)
...
test :: State -> String -> Value -> IO(Value, State) -> IO(Value, State) -> IO(Value, State)
test (st@(Exception _ _ _)) name t tf ff = return(Bad,st)
test st name (IntV 0) truth falsehood = falsehood
test st name (IntV _) truth falsehood = truth
test st name _ t f = error ("Non Int as argument to "++name++" test.")

-- Some operations expect pairs
pairLike (Exception _ _ _) name fun x = Bad
pairLike state name fun (PairV v1 v2) = fun v1 v2
pairLike state name fun v = 
  error ("function "++name++" is not passed a pair."++ show v)

charLike state name fun (CharV c) = fun c
charLike state name fun v = 
...
interpE funs vars state exp = traceG run state exp where
   ...
   run state (Pair x y) = 
     do { (v1,state1) <- interpE funs vars state x
        ; (v2,state2) <- interpE funs vars state1 y
        ; if exceptional state2
             then return(Bad,state2)
             else return(PairV v1 v2,state2)}
   run state (Fst x)    = 
     do { (v1,state1) <- interpE funs vars state x
        ; return(pairLike state1 "fst" (\ x y -> x) v1,state1)}
   run state (Snd x)    = 
     do { (v1,state1) <- interpE funs vars state x
        ; return(pairLike state1 "snd" (\ x y -> y) v1,state1)}   
   run state (Ispair x) = 
     do { (v1,state1) <- interpE funs vars state x
        ; case v1 of
