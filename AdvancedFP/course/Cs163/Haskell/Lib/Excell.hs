...
listZip :: Int -> [[b]] -> [[b]]
listZip 0 zs = []
listZip n zs = (map head zs) : listZip (n-1) (map tail zs)
...
rows xs = Tab numrows numcols (concat (map extend xs))
  where numrows = length xs
        ...
        pads transpose n xs =  map (++ replicate n "") (transpose (map visualize xs))
...
showT n (Tab height width xs) = label ++ bar ++ concatMap row xs
