-- H Forrest Alexander
-- Exercise 6 - ML&PL - modified Tableau.hs
...
-- Write a tableau style prover that checks if all of the paths are closed.
-- This starts with try2 to generate all of the paths (as a list of lists)

-- Check if any conjugate pairs exist.
-- This is simply done by checking the first element against all others,
-- discarding it and checking the 2nd against all others, etc.
-- Once an element is checked against all others,
-- we know it has no conjugates among them so it can be discarded.
anyConj :: [FormulaS] -> Bool
anyConj [_] = False                        
anyConj (x:xs) = case someConj x xs of
                      Nothing -> anyConj xs
                      Just _  -> True

-- Check if all paths of the negated formula contain a conjugate,
-- and are therefore closed.
-- If all paths are closed, report True -- the original formula is a tautology.
close x = all anyConj $ fst $ try2 x

-- TODO: 
-- Solve the problem that some universal quantified formulae may need to be used more than once.
-- For example see `f8` below.
-- Unfortunately I did not have time to finish.
 
-- Some test formulae :: FormulaS
f1 = "(forall x. O(x,x)) --> (forall x. exists y. O(x,y))"
f2 = "(forall x. O(x,x)) --> (forall x. forall y. O(x,x) | O(y,y))"
f3 = "(forall x. O(x,x)) --> (forall x. forall y. O(x,x) & O(y,y))"
f4 = "(P(a) | (exists x . P(f(x)))) --> (exists y . P(y))"
f5 = "(exists x . (P(x) | Q(x))) --> ((exists y . P(y)) | (exists y . Q(y)))"
f6 = "(exists x . forall y . P(x, y)) --> (forall y . exists x . P(x, y))" -- T
f7 = "(forall x . exists y . P(x, y)) --> (forall y . exists x . P(x, y))"
f8 = "(forall x . O(x, x)) --> (forall x . (forall y . (O(x, x) & O(y,y))))"
[p1, p2, p3, p4, p5, p6, p7, p8] = map toFormula [f1, f2, f3, f4, f5, f6, f7, f8] :: [FormulaS]
