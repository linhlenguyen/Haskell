...
andB TruthP x = x
andB x TruthP = x
andB AbsurdP x = AbsurdP
andB x AbsurdP = AbsurdP
andB x y | x==y = x
andB (LetterP x) (NotP (LetterP y)) | x==y = AbsurdP
andB (NotP (LetterP y)) (LetterP x) | x==y = AbsurdP
andB x (w@(AndP y z)) | x==y = w
andB x y = AndP x y
...
orB x AbsurdP = x
orB x y | x==y = x
orB (LetterP x) (NotP (LetterP y)) | x==y = TruthP
orB (NotP (LetterP y)) (LetterP x) | x==y = TruthP
orB x (w@(OrP y z)) | x==y = w
orB x y = OrP x y
...
equalB TruthP x = x
equalB x TruthP = x
equalB AbsurdP x = notB x
equalB x AbsurdP = notB x
equalB x y | x==y = TruthP
equalB x y = (iff x y)

iff x y = (andB x y) `orB` (andB (notB x) (notB y))

xorB TruthP x = notB x
xorB x TruthP = notB x
xorB AbsurdP x = x
xorB x AbsurdP = x
xorB x y | x==y = AbsurdP
xorB x y = notB(iff x y)

-- Fold these one level oerations over lists

orL xs = foldr orB AbsurdP (sortBy compare xs)
orOpt x y = foldr orB AbsurdP (sortBy compare (collect (OrP x y)))
  where collect (OrP x y) = collect x ++ collect y
        collect x = [x]

andL xs = foldr andB TruthP (sortBy compare xs)
andOpt x y = foldr andB TruthP (sortBy compare (collect (AndP x y)))
...
taut4 = TestCase (checkX "equal is negation of xor" 
     (taut (iff (notB(equalB 2 3)) (xorB 2 3))))
