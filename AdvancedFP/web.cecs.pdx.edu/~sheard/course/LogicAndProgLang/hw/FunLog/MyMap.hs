...
insertWithKeyM f kx x = kx `seq` go
  where
    go Tip = return(singleton kx x)
    go (Bin sy ky y l r) =
        case compare kx ky of
            LT -> do {l2 <- go l; return(balance ky y l2 r)}
            GT -> do {r2 <- go r; return(balance ky y l r2)}
            EQ -> do {y2 <- (f kx x y); return(Bin sy kx y2 l r)}            
...
fromListWithM f xs
  = fromListWithKeyM (\_ x y -> f x y) xs  
...
fromListWithKeyM :: (Monad m, Ord k) => (k -> a -> a -> m a) -> [(k, a)] -> m (Map k a)    
fromListWithKeyM f xs 
  = foldlStrictM ins empty xs
  where
    ins t (k,x) = insertWithKeyM f k x t    
...
foldlStrictM :: Monad m => (a -> b -> m a) -> a -> [b] -> m a
foldlStrictM f = go
  where
    go z []     = return z
    go z (x:xs) = z `seq` (do {y <- (f z x); go y xs})
