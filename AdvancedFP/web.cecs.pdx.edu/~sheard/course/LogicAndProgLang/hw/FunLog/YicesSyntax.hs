...
{- |
 ...
 -}
module YicesSyntax ( TypY(..) , ExpY(..) , CmdY(..), andY, orY ) where
...
andY xs = AND(concat (map andL xs))

orL (OR xs) = xs
orL y = [y]

orY xs = OR(concat (map orL xs))
...
showTypY (SUBTYPE idty e) = paren $ showString "subtype (" . showIdTyp idty . showString ") ". showsPrec 0 e
