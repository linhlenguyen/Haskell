...
---------------------------------------------------------------------------
-- YOU MUST CHANGE 'yicesExecPath' SO IT POINTS TO YICES ON YOUR MACHINE.
 
-- Inside a cygwin window I type
-- $ which yices
-- /cygdrive/c/yices-1.0.29/bin/yices
-- This translates to
 
yicesExecPath = "C:\\yices-1.0.29\\bin\\yices"

runYices = quickCheckY yicesExecPath


test = runYices ["-e"] 
        [ DEFINE ("x",VarT "int") Nothing
        , ASSERT (AND [x :< (LitI 2), x :> (LitI 0)])
        ]
  where x = VarE "x"
  ...
  do runCmdsY yp [ECHO('\n':_BEGIN_OUTPUT), CHECK, ECHO('\n':_END_OUTPUT)]
     ...
     return $
       case s of
         ...
         "unknown"-> Unknown (unlines ss)
         "unsat"  -> UnSat (unlines ss)
         _        -> InCon (s:ss)
...
quickCheckY' yPath yOpts cmds = do
     ...
     return $
       case lines out of
         ...
         "unsat"   : ss -> UnSat(unlines ss)  -- UnSat   (map read.words.tail.dropWhile (/=':').head $ ss)
         other          -> InCon   other
