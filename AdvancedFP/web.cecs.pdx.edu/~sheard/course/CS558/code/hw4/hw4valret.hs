...
interpE funs vars state exp = (traceG vars) -- this term: "(traceG vars)" enables tracing
   ...
   -- call by Value Result (results are copied back into variable actuals)             
     ...
     case lookUp funs f of
        ...
        Found (vars2,formals,body) -> 
          do { when (length args /= length formals)
             ...
             ; let callByValueResult [] [] (env,state)  = return (env,state)
                   callByValueResult (n:ns) (e:es) (env,state) =
                     do { (v,state2) <- interpE funs vars state e
                        ; let (adr,state3) = alloc v state2
                        ; (env4,state4) <- callByValueResult ns es (env,state3)
                        ; return(extend n adr env4,state4)}
             ; (vars3,state3) <- callByValueResult formals args (vars2,state)
             ; (ans,state4) <- interpE funs vars3 state3 body
             ; let copyBack [] [] state = return(ans,state)
                   copyBack (Var x : as) (f : fs) state = copyBack as fs state2
                     where (Found formalAddr) = lookUp vars3 f
                           (Found actualAddr) = lookUp vars x
                           value = access formalAddr state
                           state2 = stateStore actualAddr value state
                   copyBack (exp : as) (f:fs) state = copyBack as fs state
                        -- copyBack (TR.trace ("Warning, non var in call be value result: "++show exp) as) fs state
             ; copyBack args formals state4 }
...
test6 = parseFile progP "../hw3/lists.e3"
